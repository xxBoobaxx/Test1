...
fieldId(#field{id=Id}, Object, Opt) ->
    form:atom([Id, form:type(Object), form:kind(Opt)]);
fieldId(X, Object, Opt) ->
    form:atom([X, form:type(Object), form:kind(Opt)]).
...
extract(Object, X, Opt) ->
    ...
    io:format("Object:~p X:~p Pos:~p~n", [Object,X,Pos]),
    ...
    #panel{id = form:atom([form, Name]), class = form,
           body =
               [BuildBlock(Document, Object, Opt)
                || BuildBlock
                       <- [fun caption/3, fun fields/3,fun buttons/3]]};
...
buttons(Document, Object, Opt) ->
    ...
    #panel{id = forpreload, class = buttons, body =
        lists:foldr(fun (#but{} = But, Acc) ->
            Sources = lists:map(fun(S) -> fieldId(S,Object,Opt) end, But#but.sources),
            io:format("BUT sources: ~p~n", [Sources]),
            [#link{id =
                    ...
                    target = But#but.target, source = Sources}
            | Acc]
        end,
        [],
        Buttons)}.
...
fieldType(string, X, _Options, Object, Opt) ->
    %io:format("render string Id: ~p~n", [fieldId(X, Object, Opt)]),
